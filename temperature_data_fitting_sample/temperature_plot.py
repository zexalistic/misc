...
class Sheet:
    def __init__(self, sheet):
        ...
        self.param_meter = np.polyfit(self.meter, self.adc, 2)
        self.param_meter = np.array([round(self.param_meter[0], 2), round(self.param_meter[1], 2), round(self.param_meter[2], 2)])
        self.func = np.poly1d(self.param_meter)
        self.poly_fitting_meter = self.func(self.meter)
        param_lower_bound = np.array([self.param_meter[0], 6 * self.param_meter[0] + self.param_meter[1], 9 * self.param_meter[0] + 3 * self.param_meter[1] + self.param_meter[2]])
        func = np.poly1d(param_lower_bound)
        self.poly_lower_bound_meter = func(self.meter)
        param_upper_bound = np.array([self.param_meter[0], self.param_meter[1] - 6 * self.param_meter[0], 9 * self.param_meter[0] - 3 * self.param_meter[1] + self.param_meter[2]])
        func = np.poly1d(param_upper_bound)
        self.poly_upper_bound_meter = func(self.meter)

        self.param_die = np.polyfit(self.die, self.adc, 2)
        self.param_die = np.array([round(self.param_die[0], 2), round(self.param_die[1], 2), round(self.param_die[2], 2)])
        self.func = np.poly1d(self.param_die)
        self.poly_fitting_die = self.func(self.die)
        param_lower_bound = np.array([self.param_die[0], 6 * self.param_die[0] + self.param_die[1], 9 * self.param_die[0] + 3 * self.param_die[1] + self.param_die[2]])
        func = np.poly1d(param_lower_bound)
        self.poly_lower_bound_die = func(self.die)
        param_upper_bound = np.array([self.param_die[0], self.param_die[1] - 6 * self.param_die[0], 9 * self.param_die[0] - 3 * self.param_die[1] + self.param_die[2]])
        func = np.poly1d(param_upper_bound)
        self.poly_upper_bound_die = func(self.die)
...
sheet_name = 'volex_1_5'
sheet = df_raw.get(sheet_name).values
sheet = np.delete(sheet, 0, axis=0)  # remove nan row
volex_1_5 = Sheet(sheet)

sheet_name = 'AMPHENOL_2'
sheet = df_raw.get(sheet_name).values
sheet = np.delete(sheet, 0, axis=0)  # remove nan row
amp_2 = Sheet(sheet)

sheet_name = 'volex_2_5'
sheet = df_raw.get(sheet_name).values
sheet = np.delete(sheet, 0, axis=0)  # remove nan row
volex_2_5 = Sheet(sheet)
...
# plt.plot(amp_2.meter, amp_2.poly_fitting_meter, color='g', linestyle='solid', alpha=0.3, label=f'y={amp_2.param_meter[0]}$x^2$+{amp_2.param_meter[1]}x+{amp_2.param_meter[2]}')
...
#
# plt.figure(6)
# plt.scatter(amp_2.meter0, amp_2.adc0, color='b', linestyle='dotted', label='meter0 (amphenol 2m)')
# plt.scatter(amp_2.meter1, amp_2.adc1, color='r', linestyle='dotted', label='meter1 (amphenol 2m)')
# plt.scatter(amp_2.die0, amp_2.adc0, color='g', linestyle='dotted', label='die0 (amphenol 2m)')
# plt.scatter(amp_2.die1, amp_2.adc1, color='y', linestyle='dotted', label='die1 (amphenol 2m)')
